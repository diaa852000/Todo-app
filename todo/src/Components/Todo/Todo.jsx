...
const Todo = ({ title, content, todoId, isComplete}) => {
    ...
    return (
        <div className='dark:bg-[#1e1e1e] dark:text-white bg-white border-b p-2 text-black md:w-2/3 flex justify-between items-center cursor-pointer hover:bg-gray-100'>
            <div className='flex items-center gap-4'>
                <button 
                    type='button'
                    onClick={handleIsComplete}
                >
                    {isComplete ? <BsCheckCircle className='dark:text-white text-indigo-600'/> : <BsCircle className='dark:text-white text-indigo-600'/>}
                </button>
                <div>
                    <h2 className={`font-semibold dark:text-white text-slate-600 ${isComplete ? 'line-through' : ''}`}>{title}</h2>
                    <p className={`dark:text-white text-slate-500  ${isComplete ? 'line-through' : ''}`}>{content.substring(0,60)}</p>
                </div>
            </div>
            <div className='flex items-center gap-4'>
                <button 
